...
## 四、PEQ 算法（peq_toggle.py 内嵌 JS）

### 第一层：房间驻波校正（动态多段）
```
对 L/W/H 三维: f = n × 343 / (2 × dim),  n=1,2,3...  取 30~320Hz
mergeModes: ±5Hz 内合并, coinc = 重合次数
rawDepth(f,coinc) = 3.8 + coinc×1.0 + max(0,(250-f))/250×1.3,  clamp[-5.8, -3.5]
qOf(f) = min(9, max(6, 6+f/100))
baseGain: 大系统(ht21/ht51) ×0.4, 小系统(tv) ×0.16
LOW_CUT: 大系统30Hz起, 小系统(tv)50Hz起
用户房间 L3.1×W4.4×H2.8 → 16段
```

### 第二层：音调滑块
| 滑块 | 类型 | 频率 | Q | 增益范围 |
|------|------|------|---|---------|
| 低音 | lowshelf 搁架 | 150Hz | 0.7 | ±10dB, low=0时不加 |
| 中音 | peaking | 1350Hz | 1.0 | ±10dB |
| 高音 | peaking | 8000Hz | 1.0 | ±10dB |

> 低音滑杆是 **150Hz lowshelf 宽频搁架**，整体抬升/衰减低频，不碰每个驻波 notch 的位置和深度。

### 第三层：下混补偿带
- 300Hz peaking, Q=1.0, 增益 = 2.5 + 中音滑块值（固定+2.5dB基础补偿下混损失 + 跟随中音偏移）

### 当前配置（peq.json）
- 16段驻波(55.3~311.8Hz) + comp(300Hz,+5dB) + mid(1350Hz,+2.5dB) + hi(8000Hz,+3dB) + lowshelf(150Hz,+2.5dB) = 20段
- 生成参数: L=3.1 W=4.4 H=2.8 sys=tv low=2.5 mid=2.5 hi=3
